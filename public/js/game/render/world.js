...
function drawBloodParticle(ctx, b, sx, sy) {
  ...
  if (b.kind === 'smoke') {
    const x = b.x * sx;
    const y = b.y * sy;
    const r = b.size * scale * (1.25 + (1 - alpha) * 1.25);
    const tx = (b.x - (b.vx || 0) * 6.2) * sx;
    const ty = (b.y - (b.vy || 0) * 6.2) * sy;
    ctx.save();
    ctx.globalCompositeOperation = 'source-over';
    ctx.strokeStyle = withAlpha(color, 0.16 * alpha);
    ctx.lineWidth = Math.max(2, b.size * 0.55 * scale);
    ctx.lineCap = 'round';
    ctx.beginPath();
    ctx.moveTo(tx, ty);
    ctx.lineTo(x, y);
  ...
  if (b.kind === 'fire') {
    const x = b.x * sx;
    const y = b.y * sy;
    const r = b.size * scale * (1.35 + (1 - alpha) * 0.45);
    const tx = (b.x - (b.vx || 0) * 5.2) * sx;
    const ty = (b.y - (b.vy || 0) * 5.2) * sy;
    ctx.save();
    ctx.globalCompositeOperation = 'lighter';
    ctx.strokeStyle = withAlpha(color, 0.34 * alpha);
    ctx.lineWidth = Math.max(1.8, b.size * 0.52 * scale);
    ctx.lineCap = 'round';
    ctx.beginPath();
    ctx.moveTo(tx, ty);
    ctx.lineTo(x, y);
    ctx.stroke();
    ctx.fillStyle = withAlpha(color, 0.56 * alpha);
    ctx.beginPath();
    ctx.arc(x, y, r, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = `rgba(255, 241, 166, ${0.72 * alpha})`;
    ctx.beginPath();
