...
function drawBloodParticle(ctx, b, sx, sy) {
  ...
  if (b.kind === 'smoke') {
    ...
    const tx = (b.x - (b.vx || 0) * 8.5) * sx;
    const ty = (b.y - (b.vy || 0) * 8.5) * sy;
    ...
    ctx.lineWidth = Math.max(3, b.size * 0.8 * scale);
    ctx.lineCap = 'round';
    ctx.beginPath();
    ctx.moveTo(tx, ty);
    ctx.lineTo(x, y);
  ...
  if (b.kind === 'fire') {
    ...
    const tx = (b.x - (b.vx || 0) * 6.5) * sx;
    const ty = (b.y - (b.vy || 0) * 6.5) * sy;
    ...
    trail.addColorStop(0, 'rgba(255,74,18,0)');
    trail.addColorStop(0.45, withAlpha(color, 0.4 * alpha));
    trail.addColorStop(1, `rgba(255, 236, 150, ${0.82 * alpha})`);
    ctx.strokeStyle = trail;
    ctx.lineWidth = Math.max(2.2, b.size * 0.7 * scale);
    ctx.lineCap = 'round';
    ctx.beginPath();
    ctx.moveTo(tx, ty);
    ctx.lineTo(x, y);
